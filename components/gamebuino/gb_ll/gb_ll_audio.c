...
// 'fifo_used'") : "uint32_t inline fifo_used()" (inline SANS static)
// suit la semantique ISO C99 de "inline" -- le compilateur peut choisir
// de NE PAS emettre de definition externe pour cette fonction (il
// suppose qu'une definition "externe" existe ailleurs), contrairement a
// l'inline C++ (toujours emise si necessaire) ou a la semantique GNU89
// historique (defaut de gcc avant l'adoption large de C99). Ce fichier
// est compile en C pur (.c), et le toolchain xtensa-esp32s3-elf-gcc
// utilise ici applique par defaut la semantique C99/gnu17 -- fifo_used()
// n'etant pas inlinee a TOUS ses points d'appel (notamment dans
// gb_ll_audio_fifo_buffer_used(), autre fonction), l'edition de liens
// cherche une definition externe qui n'existe nulle part. Corrige en
// ajoutant "static" : semantique d'inline identique quel que soit le
// standard C effectif (liaison interne au fichier, definition toujours
// disponible localement si l'inlining n'a pas lieu) -- l'idiome standard
// pour de petites fonctions utilitaires en C.
